    ...
    picker.directive('dateRangePicker', ['$compile', '$timeout', '$parse', 'dateRangePickerConfig', function ($compile, $timeout, $parse, dateRangePickerConfig) {
        return {
            ...
            link: function ($scope, element, attrs, modelCtrl) {
                ...
                var _clear, _init, _initBoundaryField, _mergeOpts, _picker, _setDatePoint, _setEndDate, _setStartDate, _validate, _validateMax, _validateMin, customOpts, el, opts, _show, _hide;
                _mergeOpts = function () {
                ...
                _show = function () {
                    if (!_picker.isShowing) return _picker.show();
                    return _picker.hide();
                }
                _hide = function () {
                    if (_picker.isShowing) return _picker.hide();
                    return _picker.show();
                }
                ...
                modelCtrl.$formatters.push(function (objValue) {
                    ...
                    if (opts.singleDatePicker && objValue) {
                        return f(objValue);
                    } else if (objValue && objValue.startDate) { //MODIFIED else if (objValue.startDate)
                ...
                modelCtrl.$render = function () {
                    if (!opts.singleDatePicker && modelCtrl.$modelValue && modelCtrl.$modelValue.startDate) {
                ...
                _init = function () {
                    ...
                    for (eventType in opts.eventHandlers) {
                        results.push(el.on(eventType, function (e, picker) { //the picker property has been added
                            var eventName;
                ...
                //MODIFIED SOURCE TO ADD SHOWHIDE
                $scope.$watch(function () {
                    return $scope.showHide;
                },
                    function (n) {

                        if (n === true) return _show();
                        return _hide();
                    });
